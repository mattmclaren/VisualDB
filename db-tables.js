/**
 * VisualDB
 * Author Matthieu McClintock 2018
 */

var DbTables = (function ($, Backbone, _, undefined) {

  var dbTables;
  var windowEvent;

  function DbTables(options) {
    dbTables = this;
  ...
  DbTables.prototype.setupEvents = function () {
  ...
  DbTables.prototype.setAttributes = function (attributes) {
  ...
  DbTables.prototype.createTable = function (x, y) {
  ...
  var Base_View = Backbone.View.extend({
    ...
    mousedown: function (event) {
      if(!dbTables.selectedView)
        dbTables.selectedView = this;
      return true;
    },
  ...
  var Table_View = Base_View.extend({
    ...
    handleDrag: function (event) {
      var x = event.pageX;
      if (x > dbTables.leftPaneWidth) {
        this.x = event.pageX - 50 - dbTables.leftPaneWidth;
    ...
    setPosition: function () {
      ...
      dbTables.plumber.repaintEverything();
    },
  ...
  var Row_View = Base_View.extend({
    ...
    setUpPlumbing:function() {
      setTimeout(_.bind(function(){
        this.point1 = dbTables.plumber.addEndpoint($(this.el), {anchor: "Left"}, dbTables.plumberOptions);
        this.point2 = dbTables.plumber.addEndpoint($(this.el), {anchor: "Right"}, dbTables.plumberOptions);
      },this),1);
    },
    ...
    clickSaveButton: function () {
      ...
      console.log("Tables: ", dbTables.tables);
    },
    ...
    destroy: function (options) {
      $(this.el).remove();
      this.model.collection.remove(this.model);
      dbTables.plumber.deleteEndpoint(this.point1);
      dbTables.plumber.deleteEndpoint(this.point2);
    },
    ...
    handleDrag: function (event) {
      ...
      if(!this.isDragging){
        $(this.el).detach(); 
        $(this.el).appendTo(dbTables.$canvas);
        this.isDragging = true;
      }

      
      
      var x = event.pageX - dbTables.leftPaneWidth - 50;
      var y = event.pageY;
      if (x > dbTables.leftPaneWidth) {
    ...
    handleDrop:function(event){
      ...
      var compareX = event.pageX - dbTables.leftPaneWidth;
      var compareY = event.pageY;
      var hitTable = dbTables.tables.find(function (model) {
  ...
  var Table_Icon_View = Base_View.extend({
    ...
    handleDrop: function (event) {
      ...
      if (x > dbTables.leftPaneWidth) {
        var x = event.pageX - (dbTables.leftPaneWidth + gutterWidth);
        var y = event.pageY - 20;
        console.log('handleDrophandle'+dbTables.leftPaneWidth);
        dbTables.createTable(x, y);
